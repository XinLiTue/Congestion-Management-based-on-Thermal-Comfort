def add_opf_constraints(m, network, PLine, QLine, l, v, p, q, n_bus, t, congestion_limit, s_trafo, v_ref):
    ...
    m.addQConstr(p[0, t] * p[0, t] + q[0, t] * q[0, t] <= s_trafo**2, "trafoLimit")
    ...
    for j in range(n_bus):
        ...
        m.addConstr(p[j, t] == -P_outFlow + (P_inFlow - P_loss), name=f"BusPower{j}")
    ...
    for j in range(n_bus):
        ...
        m.addConstr(q[j, t] == -Q_outFlow + (Q_inFlow - Q_loss), name=f"BusReact{j}")
    ...
    for i in range(n_bus - 1):
        m.addConstr(l[i, t] <= (network.at[i, 'Inom'])**2, name=f"LineCurrent{i}")

    # Slack bus voltage constraint
    m.addConstr(v[0, t] == v_ref**2, "transVol")